...
-- | Parse one or more s-expressions from a string and return them as a list of
-- s-expressions (`S.Expr`).
parseSExpressions :: MonadFail m => String -> m [S.Expr]
parseSExpressions s = 
    case filter (null . snd) $ parse (only sexprs) (removeComments s) of
         (sexps, "") : _ -> return sexps
         _ -> fail "Couldn't parse s-expression."

-- | Parse a file of s-expressions
--
-- This function is in a double monad. The outer IO is necessary for reading from 
-- files, the inner, unspecified monad, allows choosing between `Maybe`, `Result`, 
-- `IO`, etc.
--
-- Here is an example combining `fromFile` with the `Result` monad (see also 
-- `Result.toIO`)
-- 
-- > do 
-- >     result <- fromFile "example1.pss"
-- >     program <- toIO $ result >>= programFromSExpressions
...
-- a space
digit, alpha, lower, upper, space :: Parser Char
...
slist :: Parser S.Expr
slist = do
    sexps <-     between lparen (sexpr `sepBy` some space) rparen
...
_scheme1 :: String
_scheme1 = "(define (segments-all-but-last nesegs)\n\
