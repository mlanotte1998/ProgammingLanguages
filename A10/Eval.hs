...
ex_program1pass = Program [Define (Sig "incr" (T.TyArrow [T.TyBase T.TyInteger, T.TyBase T.TyInteger])) (Lambda ["x"] (App [(Var "+"), (Var "x"), (Val (Integer 1))]))] (App [Var "incr", (Val (Integer 3))])
...
eval g m (Pair_Pred e) = do 
    ...
    case v of 
        ...
        _ -> return (Boolean False)         
-- New List Eval pattern matches
-- For cons needs helper to keep consistency of the list structure
eval g m (Cons e1 e2) = evalConsList g m (Cons e1 e2)
  where
    evalConsList:: Env -> Env -> Expr -> R.Result Value
    evalConsList g m (Cons e1 e2) = do 
      v1 <- eval g m e1 
...
eval g m (Cons_Pred e) = do 
  ...
  case v of 
    ConsVal _ _ -> R.Success (Boolean True) 
    _ -> R.Success (Boolean False)
-- Nil_Pred is true for just Cons    
...
eval g m (Head e) = do 
  v <- eval g m e
  case v of 
    ConsVal x _ -> R.Success x 
    _ -> R.Failure "Head called on non Cons."
-- Can grab everything but the front element of a cons list.     
eval g m (Tail e) = do 
  v <- eval g m e
  case v of 
    ConsVal _ y-> R.Success y 
    _ -> R.Failure "Tail called on non Cons."                                                                                                                    
