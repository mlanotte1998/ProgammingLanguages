...
data Value =  Closure [Signature] Expr Env
            | Integer Integer
            | Float Double
            | Boolean Bool 
            | PairVal Value Value
...
fromSExpression (S.List [S.Symbol "lambda", S.List vars, body]) = 
  Lambda (listFromSExpressionSignatureList vars) (fromSExpression body)            
...
listFromSExpressionSignatureList :: [S.Expr] -> [Signature] 
listFromSExpressionSignatureList [] = [] 
listFromSExpressionSignatureList (S.List [S.Symbol name, S.Symbol ":", t]: xs) =
        case T.fromSExpression t of 
        R.Success s -> 
            Sig name s : listFromSExpressionSignatureList xs
...
test_fromSExpression = do
  ...
  -- <= tests
    ...
    -- tests for Lambda
    test "fromSExpression Lambda 1" (fromSExpression (S.List[S.Symbol "lambda", S.List[S.List [S.Symbol "x", S.Symbol ":", S.Symbol "Integer"]], S.Symbol "x"])) (Lambda [Sig "x" (T.TyBase T.TyInteger)] (Var "x"))
    test "fromSExpression Lambda 2" (fromSExpression (S.List[S.Symbol "lambda", S.List[S.List [S.Symbol "x",  S.Symbol ":", S.Symbol "Integer"], 
      S.List[S.Symbol "y",  S.Symbol ":", S.Symbol "Integer"]], S.List[S.Symbol "+", S.Symbol "x", S.Symbol "y"]])) 
       (Lambda [Sig "x" (T.TyBase T.TyInteger), Sig "y" (T.TyBase T.TyInteger)] (App [Var "+", Var "x", Var "y"]))
    test "fromSExpression Lambda 3" (fromSExpression (S.List[S.Symbol "lambda", S.List[S.List [S.Symbol "vars", S.Symbol ":", S.Symbol "Integer"]], S.List[S.Symbol "pair", S.Symbol "x", S.Boolean True]])) 
       (Lambda [Sig "vars" (T.TyBase T.TyInteger)] (Pair (Var "x") (Val (Boolean True))))
...
toSExpression (Lambda sigs body) = S.List [S.Symbol "lambda", S.List (listOfSignaturesToSExpressionList sigs), toSExpression body]
...
listOfSignaturesToSExpressionList :: [Signature] -> [S.Expr]
listOfSignaturesToSExpressionList [] = [] 
listOfSignaturesToSExpressionList (Sig v t : xs) = S.List  [S.Symbol v, S.Symbol ":", T.toSExpression t] : listOfSignaturesToSExpressionList xs
...
-- tests for Lambda
  test "toSExpression Lambda 1" (toSExpression (Lambda [Sig "x" (T.TyBase T.TyReal)] (Var "x"))) (S.List[S.Symbol "lambda", 
   S.List[S.List [S.Symbol "x", S.Symbol ":", S.Symbol "Real"]], S.Symbol "x"])
  test "toSExpresssion Lambda 2" (toSExpression (Lambda [Sig "x" (T.TyBase T.TyReal), Sig "y" (T.TyBase T.TyReal)] (App [Var "+", Var "x", Var "y"])))
    (S.List[S.Symbol "lambda", S.List [S.List[S.Symbol "x",S.Symbol ":", S.Symbol "Real"], S.List [S.Symbol "y", S.Symbol ":", S.Symbol "Real"]], S.List[S.Symbol "+", S.Symbol "x", S.Symbol "y"]])
  test "toSExpression Lambda 3" (toSExpression (Lambda [Sig "vars" (T.TyBase T.TyInteger)] (Pair (Var "Var") (Val (Boolean True))))) 
    (S.List[S.Symbol "lambda", S.List[S.List[S.Symbol "vars", S.Symbol ":", S.Symbol "Integer"]], S.List[S.Symbol "pair", S.Symbol "Var", S.Boolean True]])
...
valueToSExpression (Closure x _ _) = S.List (S.Symbol "Lambda" : listOfSignaturesToSExpressionList x)
...
test_valueToSExpression = do
    ...
    test "valueToSExpression Closure"
        (valueToSExpression (Closure [Sig "x" (T.TyBase T.TyInteger)] (Val (Integer 5)) empty))
        (S.List [S.Symbol "Lambda",S.List [S.Symbol "x",S.Symbol ":",S.Symbol "Integer"]])    
