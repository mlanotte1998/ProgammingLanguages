...
checkFloatEquality :: Maybe Value -> Maybe Value -> Bool 
checkFloatEquality (Just (Float x)) (Just (Float y)) = if (abs (x - y) < 0.001) 
                                                             then True 
                                                             else False
...
test_checkFloatEquality = do 
    ...
    test "checkFloatEquality int with nothing" False (checkFloatEquality (Just (Integer 1)) Nothing)
    test "checkFloatEquality nothing with int" False (checkFloatEquality Nothing (Just (Integer 1)))
    test "checkFloatEquality int with int" False (checkFloatEquality (Just (Integer 1)) (Just (Integer 1)))
    test "checkFloatEquality float with nothing" False (checkFloatEquality (Just (Float 1.1)) Nothing)
    test "checkFloatEquality nothing with float" False (checkFloatEquality Nothing (Just (Float 1.1)))
    test "checkFloatEquality float with int" False (checkFloatEquality (Just (Float 1.1)) (Just (Integer 1)))
    test "checkFloatEquality int with float" False (checkFloatEquality (Just (Integer 1)) (Just (Float 1.1)))
    test "checkFloatEquality float with float should not be equal 1" False (checkFloatEquality (Just (Float 12.1)) 
     (Just (Float 12.101000001)))
    test "checkFloatEquality float with float should not be equal 2" False (checkFloatEquality (Just (Float 12.1)) 
     (Just (Float 8.0)))
    test "checkFloatEquality float with float should be equal 1" True (checkFloatEquality (Just (Float 12.1)) 
     (Just (Float 12.100000001)))
    test "checkFloatEquality float with float should be equal 2" True (checkFloatEquality (Just (Float 12.1)) 
     (Just (Float 12.1)))
...
eval g m (Pair_Pred e) = case eval g m e of 
                          Just (Pair _ _) -> Just (Boolean True)        
                          _ -> Just (Boolean False)                                                                                                        
...
evalTupleListHelper :: GlobalEnv -> Env -> [(Expr, Expr)] -> Maybe Expr -> Maybe Value
...
evalTupleListHelper g m ((t1, t2):xs) y = case eval g m t1 of 
                                         (Just (Boolean True)) -> eval g m t2 
                                         (Just (Boolean False)) -> evalTupleListHelper g m xs y
                                         _ -> Nothing 
...
test_evalTupleListHelper = do 
    ...
    test "evalTupleListHelper else case basic " (evalTupleListHelper empty empty [] (Just (Integer 5))) (Just (Integer 5))
...
test_eval = do
    -- // Boolean Tests
    test "eval Boolean  True" (eval empty empty (Boolean True)) (Just (Boolean True))

    test "eval Boolean False" (eval empty empty (Boolean False)) (Just (Boolean False))
    ...
    test "eval Float: simple Integer test" True (checkFloatEquality (eval empty empty ( Float 11.1)) (Just (Float 11.1)))
   ...
   -- // And tests 
    ...
    test "eval Number_Pred: Expr is not a integer or float simple" (eval empty empty ( Number_Pred 
     (Boolean False))) (Just (Boolean False))
    ...
    test "eval Call: call with a variable of same name in env" (eval empty [("hello", (Integer 10))] (Call "hello" [(Integer 10)]))
     (Nothing)  
    ...
    case se of 
      (S.List[S.Symbol "Program", S.List defs, e]) -> 
        case evalProgram (programFromSExpression se) of
         (Just (Integer v)) -> Just (valueToSExpression (Integer v))
         (Just (Float v)) -> Just (valueToSExpression (Float v))
         (Just (Boolean v)) -> Just (valueToSExpression (Boolean v))
         (Just (Pair e1 e2)) -> Just (valueToSExpression (Pair e1 e2))
         Nothing -> Nothing
      _ -> 
